...
func (f *scheduledCardsCollector) Run(ctx context.Context, input <-chan card.BriefCard, output chan<- card.BriefCard) error {
	var scheduledToRemindToday, remindedToday []*card.Card
	...
	scheduledCardsRemaining := f.config.ScheduledCardsCount - len(remindedToday)
...
func (f *scheduledCardsCollector) GetScheduledCards() []*card.Card {
	return f.scheduledCards
}
